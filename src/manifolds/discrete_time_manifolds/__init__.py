...
class DiscreteTimeManifold(Manifold):
    ...
    def __init__(self, d, L=100, tol=1e-2, max_iter=20000, step_size=1/8, L2=200, tol2=1e-4, max_iter2=200):
        super().__init__(d)
    ...
    def geodesic(self, x, y, t):
        ...
        if self.L > 0: # compute discrete geodesics 
            Z = self.discrete_geodesic(x, y[None])[0]
            ...
            for i,tau in enumerate(t):
                if tau < 1/self.L:
                    Y[i] = tau * (Z[0] - x) + x
                elif tau >= (self.L-1)/self.L:
                    Y[i] = (1 - (1 - tau) * self.L) * (y - Z[-1]) + Z[-1]
                else:
                    j = torch.floor(tau * self.L).int()
                    Y[i] = (tau * self.L - j) * (Z[j] - Z[j-1]) + Z[j-1]
    ...
    def log(self, x, y):
        ...
        if self.L > 0: # compute discrete geodesics 
            ...
            return self.L * (Z[:,0] - x[None])
        else: # compute straight line between end points
            return Euclidean(self.d).log(x,y)
    
    def discrete_geodesic(self, x, y): # TODO allow for initialisation Z0 + consider Newton for last steps
        """
        ...
        """
        N, _ = y.shape
        tau = torch.linspace(0.,1.,self.L+1)
        ...
        while k < self.max_iter: 
            ...
            losses = torch.zeros(N,self.L)
            losses[:,0] = self.norm(x[None], Z[None,:,0] - x[None,None])**2
            losses[:,-1] = self.norm(Z[:,-1], y[:,None] - Z[:,-1,None])[:,0]**2
            losses[:,1:-1] = self.norm(Z[:,:-1].reshape(-1,self.d), Z[:,1:].reshape(-1,1,self.d) - Z[:,:-1].reshape(-1,1,self.d)).reshape(N,self.L-2)**2
    
            # compute loss and validation
            loss = 1/2 * torch.sum(losses)
            validation = self.L * torch.sum(losses,1) - torch.sum(torch.sqrt(losses),1)**2
            if k == 0:
                validation_0 = validation.clone().max()

            if validation.max() / (validation_0 + 1e-8) < self.tol:
                break

            # compute Riemannian gradients
            loss.backward()
            gradient = Z.grad
            inverse_metric_tensor_Z = self.inverse_metric_tensor(Z.reshape(-1,self.d)).reshape((N,self.L-1,self.d,self.d))
            ...
            with torch.no_grad():  # Disable gradient tracking for parameter updates
                Z -= self.step_size * Riemannian_gradient_Z
    ...
    def exp(self, x, X):
        ...
        """
        N, _ = X.shape
        if self.L2 > 0: # compute discrete geodesics 
